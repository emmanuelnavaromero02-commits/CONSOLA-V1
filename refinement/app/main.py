...
async def mcp_invoke(body: dict, internal_service: str = Depends(verify_api_key)):
    ...
    if tool == "save_dataset":
        ...
        if str(args.get("name") or "").strip().casefold() in (
            PROTECTED_AUTHORITY_DATASETS
        ):
...
def _get_data_catalog(
    ...
    if tags:
        ...
        params.append(tags)
    if datasets:
        conditions.append("c.dataset = ANY(%s)")
    ...
    for row in cols:
        ...
        if dn not in datasets_out:
            ...
            ds_meta = (
                store.get_dataset(dn, **store_scope)
                if security_context
                else store.get_dataset(dn)
            )
            datasets_out[dn] = {
                "layer": row["layer"],
                "cartridge": row["cartridge"],
    ...
    if not tags:
        for ds_meta in store.list_datasets(**store_scope):
            ...
            if security_context and not _dataset_allowed(security_context, ds_meta):
                continue
            # The source-level regression pins this legacy mapping layout.
            # fmt: off
            datasets_out[name] = {
