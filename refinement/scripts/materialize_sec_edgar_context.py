...
DATASET_ORDER = [
    ("sec_company_metadata_latest", "silver"),
    ("sec_company_facts_normalized", "silver"),
    ("sec_company_quality", "silver"),
    ("sec_market_context", "gold"),
]


class SECMaterializationError(RuntimeError):
    pass
...
def _run_id() -> str:
    return "SEC_EDGAR_CONTEXT_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")


def _default_evidence_dir() -> Path:
    return Path(os.environ.get("OMEGA_EVIDENCE_DIR", "/tmp/omega-evidence")) / "sec-edgar-context" / _run_id()
...
class ScopedDatasetStore:
    ...
    def update_refresh(self, name: str, row_count: int) -> None:
        with self._conn() as conn, conn.cursor() as cur:
            self._set_scope(cur)
            cur.execute("UPDATE datasets SET last_refresh = NOW(), row_count = %s, updated_at = NOW() WHERE name = %s", (row_count, name))
            conn.commit()


def materialize_sec_edgar_context(
    *,
    store,
    engine,
    tenant_id: str,
    ...
    if not tenant_id or not workspace_id:
        raise SECMaterializationError("tenant_id and workspace_id are required")
    ...
    for name, expected_layer in datasets:
        ...
        if dry_run:
            ...
            item = {"name": name, "layer": expected_layer, "status": "PASS", "dry_run": False, "row_count": row_count, "storage_uri": result.get("storage_uri")}
        results.append(item)
    return {"status": "PASS", "tenant_id": tenant_id, "workspace_id": workspace_id, "datasets": results}


def _validate_dataset(ds: dict | None, name: str, expected_layer: str) -> None:
    if not ds:
        raise SECMaterializationError(f"{name} is not registered in datasets")
    if ds.get("cartridge") != "sec_edgar":
        raise SECMaterializationError(f"{name} must belong to sec_edgar")
    if ds.get("layer") != expected_layer:
        raise SECMaterializationError(f"{name} must be layer={expected_layer}")
    if not str(ds.get("sql_def") or "").strip():
        raise SECMaterializationError(f"{name} has no sql_def")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize governed SEC EDGAR context.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        summary = materialize_sec_edgar_context(
