...
DATASET_ORDER = [
    ("banxico_series_metadata_latest", "silver"),
    ("banxico_series_observations_normalized", "silver"),
    ("banxico_series_quality", "silver"),
    ("banxico_market_context", "gold"),
]


class BanxicoMaterializationError(RuntimeError):
    pass
...
def _run_id() -> str:
    return "BANXICO_CONTEXT_" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def materialize_banxico_context(
    *,
    store,
    engine,
    tenant_id: str,
    ...
    if not tenant_id or not workspace_id:
        raise BanxicoMaterializationError("tenant_id and workspace_id are required")
...
def _validate_dataset(ds: dict | None, name: str, expected_layer: str) -> None:
    if not ds:
        raise BanxicoMaterializationError(f"{name} is not registered in datasets")
    if ds.get("cartridge") != "banxico":
        raise BanxicoMaterializationError(f"{name} must belong to banxico")
    if ds.get("layer") != expected_layer:
        raise BanxicoMaterializationError(f"{name} must be layer={expected_layer}")
    if not str(ds.get("sql_def") or "").strip():
        raise BanxicoMaterializationError(f"{name} has no sql_def")
