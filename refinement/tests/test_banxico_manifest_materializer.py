...
from refinement.app.banxico_manifest import complete_manifests
...
def _manifest(entity: str, status: str, *, batch: str = "batch-a") -> bytes:
    prefix = (
        f"raw/banxico/{entity}/tenant_id=t/workspace_id=w/"
...
def test_complete_manifests_ignores_pending_corrupt_and_other_entities():
    storage = FakeStorage()
    base = "raw/banxico/series_observations/tenant_id=t/workspace_id=w/load_date=2026-07-11"
    ...
    storage.objects[
        "raw/banxico/series_metadata/tenant_id=t/workspace_id=w/load_date=2026-07-11/batch_id=x/manifest.json"
...
def test_manifest_discovery_is_scoped_by_tenant_and_workspace():
    storage = FakeStorage()
    requested = "raw/banxico/series_observations/tenant_id=t/workspace_id=w/load_date=2026-07-11"
    other = "raw/banxico/series_observations/tenant_id=t/workspace_id=other/load_date=2026-07-11"
...
def test_read_parquet_sql_uses_manifest_selected_uri_list_only():
    sql = _read_parquet_sql(["s3://lakehouse/raw/banxico/x/file.parquet"])

    assert "read_parquet(['s3://lakehouse/raw/banxico/x/file.parquet']" in sql
...
def test_banxico_freshness_slas_are_daily_and_event_only():
    slas = {item["series_id"]: item["freshness_sla_days"] for item in SERIES_CONFIG}

    assert slas == {"SF43718": 7, "SF61745": 120, "SF60648": 7, "SP68257": 7}


def test_duckdb_engine_routes_packaged_banxico_dataset_to_special_materializer(monkeypatch):
    called = {}
    ...
    monkeypatch.setattr("app.banxico_materializer.materialize_banxico_dataset", fake_materializer, raising=False)
    monkeypatch.setattr("refinement.app.banxico_materializer.materialize_banxico_dataset", fake_materializer, raising=False)
    engine = DuckDBEngine()

    result = engine.materialize(
        {
            "name": "banxico_series_metadata_latest",
            "layer": "silver",
            "cartridge": "banxico",
    ...
    assert called == {"name": "banxico_series_metadata_latest", "ctx": {"tenant_id": "t", "workspace_id": "w"}}


def test_banxico_materializer_rejects_unknown_banxico_dataset():
    with pytest.raises(ValueError, match="Unsupported Banxico dataset"):
        materialize_banxico_dataset(
            DuckDBEngine(),
            {"name": "banxico_unknown", "layer": "silver", "cartridge": "banxico"},
            {"tenant_id": "t", "workspace_id": "w"},
        )
