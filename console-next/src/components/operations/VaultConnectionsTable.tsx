...
import type { VaultAuthMethod, VaultConnection, VaultConnectionPayload, VaultSecret } from "@/lib/operations/types";
import {
...
export function VaultConnectionsTable() {
  ...
  const secrets = useVaultSecrets(scope.trim() || null);
  ...
  async function revealSecretValue(key: string) {
    ...
    try {
      const data = await revealSecret.mutateAsync({ scope, key });
  ...
  function saveSecretForm() {
    ...
    saveSecret.mutate(
      { scope: scope.trim(), key, payload: { value: secretForm.value } },
      {
        onSuccess: () => {
    ...
    <div className="space-y-4">
      <section className="rounded-lg border bg-card p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setTab("connections")} className={tabButton(tab === "connections")}>Conexiones</button>
            <button type="button" onClick={() => setTab("secrets")} className={tabButton(tab === "secrets")}>Secrets</button>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {tab === "connections" ? (
              <select value={cartridge} onChange={(event) => { setCartridge(event.target.value); setRevealedTokens({}); }} className="min-h-[44px] rounded-md border bg-background px-3 text-sm">
                {CARTRIDGES.map((item) => <option key={item.id} value={item.id}>{item.label} ({item.id})</option>)}
              </select>
            ) : (
              <input value={scope} onChange={(event) => setScope(event.target.value)} className="min-h-[44px] rounded-md border bg-background px-3 text-sm" placeholder="platform o cartucho" />
            )}
            <button
              type="button"
      ...
      {tab === "connections" ? (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
              ...
              <div className="overflow-x-auto rounded-lg border">
                <table className="w-full text-sm">
                  <thead className="bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
                    <tr><th className="px-3 py-2">Conn ID</th><th className="px-3 py-2">Base URL</th><th className="px-3 py-2">Auth</th><th className="px-3 py-2">Token</th><th className="px-3 py-2">Acciones</th></tr>
                  </thead>
                  <tbody>
                    {rows.map((conn, index) => {
          ...
          <ConnectionForm form={connForm} editingId={editingConnId} setForm={setConnForm} onSave={saveConnectionForm} onCancel={resetConnectionForm} saving={saveConnection.isPending} />
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
          <section className="space-y-3 rounded-lg border bg-card p-4">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-base font-semibold">Secrets de {scope || "scope"}</h2>
            ...
            {secrets.isError ? (
              ...
              <div className="overflow-x-auto rounded-lg border">
                <table className="w-full text-sm">
                  <thead className="bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
                    <tr><th className="px-3 py-2">Key</th><th className="px-3 py-2">Valor</th><th className="px-3 py-2">Acciones</th></tr>
                  </thead>
                  <tbody>
          ...
          <SecretFormPanel form={secretForm} editingKey={editingSecretKey} setForm={setSecretForm} onSave={saveSecretForm} onCancel={resetSecretForm} saving={saveSecret.isPending} scope={scope} />
        </div>
      )}
    </div>
  );
