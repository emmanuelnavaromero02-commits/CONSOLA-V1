...
import { ChevronDown, Download, Plug, Plus, RefreshCw, Upload } from "lucide-react";
...
const PROBE_DOT: Record<string, string> = {
  operational: "bg-success motion-safe:animate-pulse",
  degraded: "bg-warning",
...
export function ProbeBadge({ probe, testId = "cartridge-status" }: { probe: CartridgeProbe; testId?: string }) {
  ...
  return (
    <span data-testid={testId} data-status={status} className="inline-flex flex-wrap items-center gap-2 text-xs">
      <span
        title={detail ?? label}
        className={cn(
          "inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border px-2 py-0.5 font-medium",
          PROBE_TONE[status] ?? "border-border bg-muted text-muted-foreground",
        )}
      >
        <span aria-hidden className={cn("h-2 w-2 rounded-full", PROBE_DOT[status] ?? "bg-muted-foreground")} />
        {label}
      </span>
  ...
  return (
    <section aria-label="Cartucho activo" className="min-w-0 flex-1">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <div className="flex min-h-[44px] items-center gap-2 self-end" aria-live="polite">
