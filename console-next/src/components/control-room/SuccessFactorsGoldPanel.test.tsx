...
describe("SuccessFactorsGoldPanel", () => {
  ...
  it("treats sources without status and without data_readiness as missing, never ready", () => {
    const markup = renderToStaticMarkup(
      <SuccessFactorsGoldPanel
        payload={{ widgets: [] }}
        loading={false}
    ...
    // La fuente sin evidencia de materialización no cuenta como cobertura lista.
    expect(markup).toContain("Dataset no materializado");
    expect(markup).toContain("0/1");
    expect(markup).not.toContain("1/1 listos");
  });

  it("does not brand the panel with a hardcoded tenant name", () => {
    const markup = renderToStaticMarkup(
      <SuccessFactorsGoldPanel
        payload={{ widgets: [{ id: "employee_360", title: "Employee 360", value: 1288 }] }}
        loading={false}
        error=""
        sources={[source]}
        decisionModel={decisionModel}
    ...
    expect(markup).not.toContain("FEMSA");
    expect(markup).toContain("SuccessFactors");
  });

  it("shows N/D instead of presenting the source row-count sum as a business metric", () => {
    const recruiting: SourceStatus = {
      ...source,
      module: "Recruiting",
      count: 4000,
    };
    const candidates: SourceStatus = {
      ...source,
      module: "Candidate Pipeline",
      count: 321,
    };
    const markup = renderToStaticMarkup(
      <SuccessFactorsGoldPanel payload={{ widgets: [] }} loading={false} error="" sources={[recruiting, candidates]} />,
    );

    // Sin widget de negocio no se disfraza la suma de registros como métrica.
    expect(markup).toContain("N/D");
    expect(markup).not.toContain("4,321");
    expect(markup).not.toContain("4321");
  });

  it("renders the Workforce Trends contract status next to the block title", () => {
    const talent: SfTalentKpisPayload = {
      ...
      workforce_trends: {
        ...
        kpis: { active_headcount: null, avg_tenure_months: null, attrition_rate: null, history_months: null },
        series: { months: [], headcount: [], avg_tenure_months: [], attrition_rate: [] },
      },
    };
    const markup = renderToStaticMarkup(
      <SuccessFactorsGoldPanel payload={{ widgets: [] }} loading={false} error="" sources={[source]} talent={talent} />,
    ...
    expect(markup).toContain("Workforce Trends");
    expect(markup).toContain("En espera de datos");
  });

  it("shows N/D for absent talent counters instead of fabricated zeros", () => {
    const talent: SfTalentKpisPayload = {
      readiness: { status: "partial" },
      widgets: [],
      signals: [{ id: "s1", title: "Señal sin conteo", recommendation: "Revisar" }],
      blockers: [],
    };
    const markup = renderToStaticMarkup(
      <SuccessFactorsGoldPanel payload={{ widgets: [] }} loading={false} error="" sources={[source]} talent={talent} />,
    ...
    expect(markup).toContain("N/D/N/D");
    expect(markup).toContain("N/D afectados");
    expect(markup).toContain("cobertura de insuficiencia no informada");
    expect(markup).not.toContain("0 con información insuficiente");
  });
});
