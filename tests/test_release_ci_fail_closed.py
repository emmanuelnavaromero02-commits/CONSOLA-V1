...
def _recovery_remote(
    ...
    failed_218 = _test_commit(seed, "failed .218")
    assert (
        _test_git(seed, "tag", "-a", "v1.45.218-beta", "-m", "failed .218").returncode
        == 0
    )
    failed_218_object = _test_git_output(seed, "rev-parse", "refs/tags/v1.45.218-beta")
    failed_219 = _test_commit(seed, "failed .219")
    assert (
        _test_git(seed, "tag", "-a", "v1.45.219-beta", "-m", "failed .219").returncode
        == 0
    )
    failed_219_object = _test_git_output(seed, "rev-parse", "refs/tags/v1.45.219-beta")
    failed_220 = _test_commit(seed, "failed .220")
    assert (
        _test_git(seed, "tag", "-a", "v1.45.220-beta", "-m", "failed .220").returncode
        == 0
    )
    failed_220_object = _test_git_output(seed, "rev-parse", "refs/tags/v1.45.220-beta")
    _test_commit(seed, "recovery .221")
    assert (
        _test_git(seed, "tag", "-a", "v1.45.221-beta", "-m", "recovery .221").returncode
        == 0
    )
...
def test_previous_release_selection_requires_canonical_release_evidence():
    ...
    assert binding["run"].count("+refs/tags/") == 13
    assert (
        binding["run"].count(
            "for name in current failed-0 failed-1 failed-2 failed-3 failed-4 failed-5 failed-6 failed-7 failed-8 failed-9 failed-10 base; do"
        )
        == 2
    )
    ...
    for name in (
        ...
        assert (
            _test_git(
                ...
                "show-ref",
                "--verify",
                f"refs/omega-release-authority/v1.45.221-beta/{name}",
            ).returncode
            != 0
        )
...
def test_remote_recovery_binding_replaces_stale_checkout_with_moved_tag(
    ...
    assert result.returncode == 0, result.stderr
    authority = "refs/omega-release-authority/v1.45.221-beta/failed-0"
...
digest='sha256:f7e035122f930f0c8e120447513bbc550809fd4e909e4304a203fd6e605b1dad'
ref="${tag}@${digest}"
case "${1:-}" in
  pull) [[ "${2:-}" == "${ref}" ]] ;;
  tag) [[ "${2:-}" == "${ref}" && "${3:-}" == "${tag}" ]] ;;
