...
HINTS = REPO_ROOT / "cartridges" / "sap_s4hana" / "hints" / "assistant.md"
AGENTS_MIGRATION = REPO_ROOT / "infra" / "init" / "86_sap_s4hana_agents_seed.sql"
CARTRIDGE_SEED = REPO_ROOT / "cartridges" / "sap_s4hana" / "config" / "seed.sql"
...
AGENT_SLUGS = ["sap_s4hana_analista_comercial", "sap_s4hana_controller_financiero"]
REQUIRED_SECTIONS = [
...
def test_hints_references_existing_kbs():
    referenced = set(re.findall(r"kb_sap_s4hana_[a-z0-9_]+", _hints()))
    assert referenced, "hints references no kb_sap_s4hana_* KB"
    real = _kb_ids()
...
def test_hints_references_existing_apps():
    referenced = set(re.findall(r"sap_s4hana_[a-z_]+(?:overview|dashboard)", _hints()))
...
def test_agent_slugs_prefixed():
    for slug in AGENT_SLUGS:
        assert slug.startswith("sap_s4hana_"), f"{slug} not prefixed sap_s4hana_"
...
def test_s4hana_inventory_blocks_a_to_e():
    ...
    assert len([k for k in kb_ids if k.startswith("kb_sap_s4hana_")]) == 8
    sql81 = MIGRATION_81.read_text(encoding="utf-8")
    n_datasets = len(re.findall(r"\$seed\$[a-z0-9_]+\$seed\$,\s*\$seed\$(?:silver|gold)\$seed\$", sql81))
    assert n_datasets == 27, f"expected 27 datasets, got {n_datasets}"
    apps = sorted(p.stem for p in APPS_DIR.glob("*.html"))
    assert apps == ["sap_s4hana_finance_dashboard", "sap_s4hana_sales_overview"]
