"""RED acceptance contracts for the v1.45.216 transition bridge."""
...
CURRENT_TAG = "v1.45.216-beta"
BASE_TAG = "v1.45.209-beta"
AUTHORITY = "refs/omega-release-authority/v1.45.216-beta"
EXPECTED_MARKERS = (
    (
        "v1.45.210-beta",
...
def _transition_history(
    ...
    recovery = _commit(tmp_path, "recovery release .216")
    _annotated_tag(tmp_path, CURRENT_TAG, "recovery release .216", recovery)
...
def test_f216_ledger_has_six_exact_failed_markers_and_exact_f215_pin() -> None:
...
def test_f216_workflow_binds_all_eight_remote_refs_in_one_atomic_fetch() -> None:
    ...
    assert source.count("+refs/tags/") == 8
    assert (
        source.count(
            "for name in current failed-0 failed-1 failed-2 failed-3 "
            "failed-4 failed-5 base; do"
        )
        == 2
    )


def test_f216_bridge_accepts_exact_six_marker_chain_without_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_f216_bridge_fails_closed_when_f215_authority_is_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
    _git(history.repo, "update-ref", "-d", f"{history.authority}/failed-5")
...
def test_f216_bridge_fails_closed_when_f215_annotated_tag_is_recreated(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
    ...
    recreated = _annotated_tag(history.repo, tag, "attacker-recreated .215", commit)
    assert recreated != original_object
    _git(
        history.repo,
        "update-ref",
        f"{history.authority}/failed-5",
        f"refs/tags/{tag}",
    )
...
def test_f216_bridge_fails_closed_when_f215_marker_is_lightweight(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
...
def test_f216_bridge_fails_closed_on_f215_canonical_evidence_contradiction(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
    f215_tag, _f215_object, f215_commit = history.markers[-1]
    ...
    with pytest.raises(ReleaseTrustError, match="contradictory canonical evidence"):
        selector.select_previous_release(
            ...
            trust_verifier=lambda tag, commit: (
                tag == f215_tag and commit == f215_commit
            ),
        )
