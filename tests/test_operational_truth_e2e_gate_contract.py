...
def test_the_e2e_stack_provides_every_per_pair_key_console_requires_to_start():
    """Console refuses to start without the key of every cartridge it names in
    ``auth.py`` (production-style startup). The e2e stack runs console that way,
    so a cartridge added to that map but not to the e2e script and compose makes
    the whole gate fail with an exited container, far from the cause (that is
    what happened when sap_b1 was wired in)."""
    import re
