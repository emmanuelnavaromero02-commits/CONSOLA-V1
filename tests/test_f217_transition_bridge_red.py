"""RED acceptance contracts for the v1.45.219 transition bridge."""
...
def _transition_history(
    ...
    *,
    ...
    if interpose_before_recovery:
        _commit(tmp_path, "untrusted commit between .218 and .219")
...
def test_f219_workflow_binds_all_eleven_remote_refs_in_one_atomic_fetch() -> None:
    ...
    names = (
        ...
        "failed-5 failed-6 failed-7 failed-8 base; do"
    )
    assert source.count(names) == 2


def test_f219_bridge_accepts_exact_nine_marker_direct_chain_without_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_f219_bridge_requires_recovery_directly_atop_f218(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_f219_bridge_fails_closed_when_f218_annotated_tag_is_moved(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
...
def test_f219_bridge_fails_closed_when_f218_marker_is_lightweight(
    ...
    _git(history.repo, "update-ref", f"{history.authority}/failed-8", commit)
