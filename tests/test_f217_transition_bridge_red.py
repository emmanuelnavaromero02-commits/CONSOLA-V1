"""RED acceptance contracts for the v1.45.218 transition bridge."""
...
def _transition_history(
    ...
    *,
    ...
    if interpose_before_recovery:
        _commit(tmp_path, "untrusted commit between .217 and .218")
...
def test_f218_workflow_binds_all_ten_remote_refs_in_one_atomic_fetch() -> None:
    ...
    names = (
        ...
        "failed-5 failed-6 failed-7 base; do"
    )
    assert source.count(names) == 2


def test_f217_bridge_accepts_exact_seven_marker_direct_chain_without_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_f217_bridge_requires_recovery_directly_atop_f216(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_f217_bridge_fails_closed_when_f216_authority_is_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
...
def test_f217_bridge_fails_closed_when_f216_annotated_tag_is_moved(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
...
def test_f217_bridge_fails_closed_when_f216_marker_is_lightweight(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
    _tag, _object, commit = history.markers[-1]
    _git(history.repo, "update-ref", f"{history.authority}/failed-7", commit)
...
def test_f217_bridge_fails_closed_on_f216_canonical_evidence_contradiction(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    history = _transition_history(tmp_path, monkeypatch)
    f216_tag, _f216_object, f216_commit = history.markers[-1]
    ...
    with pytest.raises(ReleaseTrustError, match="contradictory canonical evidence"):
        selector.select_previous_release(
            ...
            trust_verifier=lambda tag, commit: (
                tag == f216_tag and commit == f216_commit
            ),
        )
