...
def _signed(ctx: dict, *, signed_at: int | None = None, key: str = SIGNING_KEY) -> dict:
    ...
    signed["_signature"] = hmac.new(key.encode("utf-8"), raw, hashlib.sha256).hexdigest()
    return signed
...
def test_vault_rejects_security_context_signing_key_reused_as_transport_key(monkeypatch):
    main = _load_service("vault", monkeypatch)
