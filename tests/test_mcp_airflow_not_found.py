...
@pytest.mark.anyio
async def test_airflow_get_task_logs_404_returns_empty_logs(monkeypatch):
    airflow = _load_airflow_tools(monkeypatch)
    monkeypatch.setattr(airflow.httpx, "AsyncClient", lambda **_kwargs: FakeClient())

    result = await airflow.airflow_get_task_logs("sap_successfactors_extract", "manual__missing", "extract")
...
@pytest.mark.anyio
async def test_airflow_list_dag_runs_404_returns_empty_runs(monkeypatch):
    airflow = _load_airflow_tools(monkeypatch)
    monkeypatch.setattr(airflow, "_client", lambda: FakeClient())

    result = await airflow.airflow_list_dag_runs("missing_dag")
