"""Documentary and seal contracts carried into the v1.45.222 successor."""
...
def test_f222_version_and_canonical_successor_target_are_exact() -> None:
    assert (REPO / "VERSION").read_text(encoding="utf-8").strip() == ("1.45.222-beta")
    document = _evidence()
    compact = " ".join(document.split())
    assert "canonical successor `v1.45.222-beta`" in compact
    assert "v1.45.210-beta` through `v1.45.220-beta`" in compact
    assert (
