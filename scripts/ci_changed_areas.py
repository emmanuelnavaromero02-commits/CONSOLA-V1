...
MIGRATION_CONTROL_PATHS = frozenset(
    {
...
PY_RUNTIME_EXACT_PATHS = frozenset(
    {
...
def _root_test_targets(files: list[str]) -> str:
    ...
    if _any(files, r"^scripts/gcp/", r"^infra/terraform-gcp/"):
        targets.update(
            {
    ...
    if any(path in MIGRATION_CONTROL_PATHS for path in files) or _any(
        files,
        r"^infra/migrations/manifests/",
        r"^infra/init/99zz[tu-z]_",
    ):
        targets.update(
            {
...
def _flags(files: list[str]) -> dict[str, bool | str]:
    ...
    infra = any(path in MIGRATION_CONTROL_PATHS for path in files) or _any(
        files,
        r"^infra/",
        r"^\.github/workflows/",
    ...
    release_full_stack = (
        compose
        or any(path in MIGRATION_CONTROL_PATHS for path in files)
        or _any(files, r"^infra/migrations/manifests/", r"^infra/init/.*\.sql$")
        or _any(
            full_stack_files,
