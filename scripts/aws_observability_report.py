...
def _http_json(url: str) -> tuple[str, str]:
    ...
    return ("PASS" if result.returncode == 0 else "FAIL", body)
...
emit(){ local e="${4:-}"; e="${e//$'\t'/ }"; e="${e//$'\r'/ }"; e="${e//$'\n'/ }"; printf 'OMEGA_OBS_CHECK\t%s\t%s\t%s\t%s\n' "$1" "$2" "$3" "$e"; }
env_value(){ [ -f "${DEPLOY_DIR}/.env" ] && awk -F= -v key="$1" '$1 == key {print substr($0, index($0, "=") + 1)}' "${DEPLOY_DIR}/.env" | tail -n 1 | sed "s/^[ '\"]//; s/[ '\"]$//"; }
compose_files(){ printf -- '-f docker-compose.aws.yml '; [ "$(env_value DEPLOY_CARTRIDGES_SAME_HOST)" = "true" ] && [ -f docker-compose.cartridges.yml ] && printf -- '-f docker-compose.cartridges.yml '; }
docker_health(){ docker inspect "$1" --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' 2>/dev/null || true; }
cd "$DEPLOY_DIR"
...
case "$writeback_state" in disabled) emit "release" "external writeback disabled" "PASS" "disabled";; *) emit "release" "external writeback disabled" "FAIL" "state=$writeback_state";; esac
for item in "console|http://127.0.0.1:8000/readyz|" "workspace|http://127.0.0.1:8001/healthz|" "refinement|http://127.0.0.1:8500/healthz|" "superset|http://127.0.0.1:8088/health|"; do
  IFS='|' read -r name url fallback <<< "$item"
  if curl -fsS --max-time 8 "$url" >/dev/null 2>&1; then
    emit "internal-health" "$name" "PASS" "primary=200"
  elif [ -n "$fallback" ] && curl -fsS --max-time 8 "$fallback" >/dev/null 2>&1; then
    emit "internal-health" "$name" "PASS" "fallback=200"
  else
    emit "internal-health" "$name" "FAIL" "unreachable"
  fi
done
for item in "vault|mode_vault" "mcp-infra|mode_mcp_infra"; do
  IFS='|' read -r name container <<< "$item"; state="$(docker_health "$container")"
  [ "$state" = "healthy" ] || [ "$state" = "running" ] && emit "internal-health" "$name" "PASS" "docker_health=$state" || emit "internal-health" "$name" "FAIL" "docker_health=${state:-missing}"
done
...
docker compose $(compose_files) exec -T postgres pg_isready -U postgres -d modecissions >/dev/null 2>&1 && emit "db" "operational postgres" "PASS" "pg_isready ok" || emit "db" "operational postgres" "FAIL" "pg_isready failed"
docker compose $(compose_files) exec -T postgres_gold pg_isready -U postgres -p 5433 -d modecissions_gold >/dev/null 2>&1 && emit "db" "gold postgres" "PASS" "pg_isready ok" || emit "db" "gold postgres" "FAIL" "pg_isready failed"
redis_ping="$(docker compose $(compose_files) exec -T redis redis-cli ping 2>/dev/null || true)"
[ "$redis_ping" = "PONG" ] && emit "cache" "redis ping" "PASS" "PONG" || emit "cache" "redis ping" "FAIL" "${redis_ping:-missing}"
...
bucket="$(env_value S3_BUCKET_NAME)"; region="$(env_value AWS_REGION)"; region="${region:-us-east-1}"
if command -v aws >/dev/null 2>&1 && [ -n "$bucket" ]; then
  backups="$(aws s3 ls "s3://${bucket}/backups/" --region "$region" 2>/dev/null | tail -n 1 | awk '{print $2}' | tr -d '/' || true)"
  lake_objects="$(aws s3 ls "s3://${bucket}/" --region "$region" --recursive 2>/dev/null | grep -v '/backups/' | wc -l | tr -d ' ' || true)"
  ...
  emit "lakehouse" "s3 bucket configured" "BLOCKED" "aws cli or S3_BUCKET_NAME unavailable"
fi
...
def main(argv: list[str] | None = None) -> int:
    ...
    remote = send_ssm_command_lines(
        region=args.region,
        instance_id=instance_id,
        commands=_remote_script().splitlines(),
