...
MAX_INLINE_SCRIPT_BYTES = int(os.environ.get("OMEGA_SSM_MAX_INLINE_SCRIPT_BYTES", "6000"))
SENSITIVE_PATTERNS = [
...
def aws_json(args: list[str], *, region: str = DEFAULT_REGION, timeout: int = 60) -> dict[str, Any]:
    cmd = ["aws", "--region", region, *args, "--output", "json"]
    result = run_local(cmd, timeout=timeout)
    if result.returncode != 0:
        output = "\n".join(part for part in (result.stdout, result.stderr) if part)
        raise RuntimeError(redact(output) or f"AWS CLI failed with exit code {result.returncode}")
    try:
        return json.loads(result.stdout or "{}")
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"AWS CLI returned non-JSON output: {redact(result.stdout)[:500]}") from exc


def resolve_instance_id(region: str = DEFAULT_REGION, explicit: str | None = None) -> str:
    if explicit:
        return explicit
    env_value = os.environ.get("AWS_APP_INSTANCE_ID") or os.environ.get("OMEGA_AWS_APP_INSTANCE_ID")
    if env_value:
        return env_value
...
def _encoded_bash_commands(script: str, *, chunk_size: int = 3500) -> list[str]:
    ...
    for index in range(0, len(encoded), chunk_size):
        ...
        commands.append("cat >> \"$tmp_b64\" <<'OMEGA_SSM_CHUNK'\n" + chunk + "\nOMEGA_SSM_CHUNK")
    commands.extend(
        [
...
def _run_ssm_commands(
    *,
    ...
    try:
        with params_file:
            json.dump({"commands": commands, "executionTimeout": [str(timeout_seconds)]}, params_file)
        sent = aws_json(
            [
                "ssm",
                "send-command",
...
def _send_large_ssm_script(
    *,
    ...
    upload = _run_ssm_commands(
        ...
        commands=[
            "set -eu",
            f"rm -f {remote_script} {remote_b64}",
            f": > {remote_b64}",
        ],
    )
    ...
    for index in range(0, len(encoded), chunk_size):
        ...
        upload = _run_ssm_commands(
            ...
            commands=[
                "set -eu",
                f"cat >> {remote_b64} <<'OMEGA_SSM_LARGE_CHUNK'\n{chunk}\nOMEGA_SSM_LARGE_CHUNK",
            ],
        )
        if upload.status != "Success" or upload.response_code != 0:
            return upload
    ...
    return _run_ssm_commands(
        ...
        commands=[
            "set -eu",
            f"trap 'rm -f {remote_script} {remote_b64}' EXIT",
