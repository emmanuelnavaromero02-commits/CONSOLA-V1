...
def _capture_runs(monkeypatch):
    ...
    monkeypatch.setattr(routes_skills, "run_company_facts", _fake_run)
...
def test_body_tenant_mismatch_is_rejected(monkeypatch, signing_env):
    ...
    with pytest.raises(HTTPException) as excinfo:
        routes_skills.run_incremental(
            "company_facts", body=body, conn_id=None, x_security_context=header
        )
...
def test_unsigned_body_context_never_routes_scope(monkeypatch, signing_env):
    ...
    with pytest.raises(HTTPException) as excinfo:
        routes_skills.run_incremental(
            "company_facts", body=body, conn_id=None, x_security_context=None
        )
    assert excinfo.value.status_code == 403
    assert calls == []
...
def test_scope_resolves_only_from_verified_signed_context(monkeypatch, signing_env):
    ...
    result = routes_skills.run_incremental(
        "company_facts", body={}, conn_id=None, x_security_context=header
    )
...
def test_retention_dry_run_body_tenant_mismatch_is_rejected(monkeypatch, signing_env):
    from app.api import routes_skills

    header = json.dumps(_signed_context(), ensure_ascii=False)
    body = {"tenant_id": "tenant-b", "workspace_id": "ws-a"}
    with pytest.raises(HTTPException) as excinfo:
        routes_skills.sec_bronze_dry_run(body=body, x_security_context=header)
    assert excinfo.value.status_code == 403
    assert "tenant_id" in str(excinfo.value.detail)


def test_retention_dry_run_requires_signed_context(monkeypatch, signing_env):
    from app.api import routes_skills

    body = {"tenant_id": "tenant-b", "workspace_id": "ws-b"}
    with pytest.raises(HTTPException) as excinfo:
        routes_skills.sec_bronze_dry_run(body=body, x_security_context=None)
    assert excinfo.value.status_code == 403
