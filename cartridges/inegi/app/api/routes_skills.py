...
def list_skills() -> dict:
    return {
        "service": "inegi",
        "skills": [
            _skill("/skills/test_connection", "POST", "Validate INEGI metadata preflight."),
            _skill("/skills/run_incremental/series_observations", "POST", "Run incremental INEGI Bronze ingestion."),
            _skill("/skills/run_full_load/series_observations", "POST", "Run full INEGI Bronze ingestion."),
        ],
    }
...
def test_connection(
    ...
    try:
        ...
        evidence = validate_metadata(
            INEGIClient(conn_id=conn_id, security_context=x_security_context),
            load_series_configs(),
        )
