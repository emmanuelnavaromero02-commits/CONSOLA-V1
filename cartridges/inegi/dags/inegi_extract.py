...
def _response_error(response: httpx.Response) -> str:
    ...
    if len(text) > 500:
        text = f"{text[:500]}..."
    return f"INEGI cartridge request failed status={response.status_code} body={text}"
...
@dag(
    dag_id="inegi_extract",
    description="Manual INEGI SIE Bronze-only extraction",
    schedule=None,
    catchup=False,
    max_active_runs=1,
    default_args=default_args,
    tags=["inegi", "bronze", "extract"],
    params={
