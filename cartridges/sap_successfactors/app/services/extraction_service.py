...
# Guards against SAP SuccessFactors clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP SuccessFactors's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP SuccessFactors entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP SuccessFactors, then split into batches.
