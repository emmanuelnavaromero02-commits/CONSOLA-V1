"""
sap_successfactors_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP SuccessFactors API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SF_ANALYTICS={"conn_type":"http","host":"https://na5.sap_successfactors.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_successfactors_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_successfactors' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    ...
    description="Extrae una entidad de SAP SuccessFactors → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_successfactors", "bronze", "extract"],
    params={
...
def sap_successfactors_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP SuccessFactorsClient(base_url, token)
