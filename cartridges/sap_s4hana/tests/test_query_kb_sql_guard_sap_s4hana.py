...
PREFIXES = (
    PREFIX,
    "s3://lakehouse/silver/sap_s4hana/",
    "s3://lakehouse/gold/sap_s4hana/",
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "SELECT * FROM read_parquet('s3://lakehouse/gold/sap_s4hana/revenue_by_customer/*.parquet')",
    ],
)
...
def test_query_kb_error_response_does_not_leak_resolved_sql_or_paths(monkeypatch):
    ...
    class LeakyConnection:
        def execute(self, sql: str):
            raise RuntimeError(f"boom while reading {sql} from s3://lakehouse/raw/sap_s4hana/User/x.parquet")
    ...
    result = mcp_server.query_kb("SELECT * FROM read_parquet('s3://{bucket}/raw/sap_s4hana/User/*.parquet')")
...
def test_existing_s4hana_kb_sql_passes_guard():
    repo_root = Path(__file__).resolve().parents[3]
    kb_path = repo_root / "cartridges" / "sap_s4hana" / "app" / "config" / "knowledge_bits.yaml"
