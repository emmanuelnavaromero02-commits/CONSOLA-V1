...
can inspect, extract, and query SAP S/4HANA data without writing custom code.
...
mcp = FastMCP(
    name="sap_s4hana",
    instructions=(
        "You have access to the SAP S/4HANA workforce-management cartridge. "
...
def list_entities() -> list[dict[str, Any]]:
    """
    List all SAP S/4HANA entities with their extraction mode, watermark field,
...
def get_schema(entity: str) -> dict[str, Any]:
    """
    Return the configuration schema for a SAP S/4HANA entity including field list,
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    """
    Preview the most recent rows for a SAP S/4HANA entity from Bronze (MinIO Parquet).
...
async def extract(
    ...
    """
    [BATCH — async] Trigger extraction of a SAP S/4HANA entity into Bronze (MinIO Parquet).
...
async def extract_all(mode: str = "incremental") -> dict[str, Any]:
    """
    [BATCH — async] Extrae TODAS las entidades de SAP S/4HANA en paralelo (máx 4 simultáneas).
...
def list_kbs() -> list[dict[str, Any]]:
    """
    List all Knowledge Bits defined for the SAP S/4HANA cartridge, including
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    ...
    allowed_prefix = f"s3://{settings.minio_bucket}/raw/sap_s4hana/"
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "WHERE cartridge_id='sap_s4hana' AND enabled=TRUE"
                )
                rows = cur.fetchall()
        finally:
            conn.close()
