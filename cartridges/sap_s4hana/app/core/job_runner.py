...
async def cleanup_stale() -> None:
    ...
    try:
        ...
        await pool.execute(
            ...
            "finished_at=NOW() WHERE status='running' AND tool LIKE 'sap_s4hana__%'"
        )
    except Exception:
        pass
...
async def _log(
    ...
    try:
        ...
        await pool.execute(
            ...
            "VALUES ($1, 'sap_s4hana', $2, $3, $4, $5::jsonb)",
...
async def create_extract_job(
    ...
    - Si AIRFLOW_URL está configurado: delega al DAG sap_s4hana_extract en Airflow.
    ...
    await _insert(job_id, "sap_s4hana__extract", args)
...
async def create_extract_all_job(mode: str = "incremental") -> dict:
    ...
    """
    job_id = str(uuid.uuid4())[:8]
    await _insert(job_id, "sap_s4hana__extract_all", {"mode": mode})
...
async def list_jobs(limit: int = 10) -> list[dict]:
    ...
    rows = await pool.fetch(
        "SELECT * FROM jobs WHERE tool LIKE 'sap_s4hana__%' "
...
async def _trigger_silver_refresh(entity: str) -> None:
    ...
    """
    source = f"raw/sap_s4hana/{entity}"
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        response = await client.post(
            ...
            headers={
                "x-api-key": api_key,
                "x-internal-service": "cartridge-sap_s4hana",
            },
            json={
                "source": source,
                "security_context": {
                    "trusted": True,
                    "source": "cartridge-sap_s4hana",
                    "role": "admin",
...
async def _trigger_airflow(
    ...
    """POST to Airflow REST API to trigger the sap_s4hana_extract DAG."""
