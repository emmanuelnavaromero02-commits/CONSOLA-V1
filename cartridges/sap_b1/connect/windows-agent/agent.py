...
from app.services import b1_queries, b1_reader, bronze_parquet  # noqa: E402
...
def load_config(path: Path | None, env: Mapping[str, str] | None = None) -> AgentConfig:
    ...
    return AgentConfig(
        ...
        source=_source_config(_section(data, "source"), env),
...
def cmd_extract_all(config: AgentConfig, log: logging.Logger, args: argparse.Namespace) -> int:
    ...
    return _run_entities(config, log, entities, mode=args.mode, from_date=None, to_date=None, output_dir=args.output_dir)
