-- sap_b1_customer_crosswalk  (silver)  cartridge: sap_b1
-- sources: ["raw/sap_b1/OCRD", "raw/sap_b1/IntercompanyPartners"]
-- description: One customer identity across the companies: external customers (CardType C, not a group company) keyed by their normalized tax id (RFC) when it is valid and not the generic public RFC, otherwise by company and code; companies_sharing_key counts the companies that sell to the same customer.
...
customers AS (
    ...
    WHERE CAST(o.CardType AS VARCHAR) = 'C' AND p.CardCode IS NULL
),
classified AS (
    SELECT *,
...
keyed AS (
    SELECT *,
           CASE WHEN rfc_valid AND NOT rfc_generic THEN 'RFC:' || rfc_norm
                ELSE company || ':' || card_code END                  AS customer_key,
...
SELECT
    ...
    customer_key,
    match_method,
    COUNT(DISTINCT company) OVER (PARTITION BY customer_key)          AS companies_sharing_key
FROM keyed
