...
-- description: purchase order lines with their header (OPOR/POR1, ObjType 22): amounts in document, local and system currency before and after the document (footer) discount, the sales commission the line carries, the cost the line carries, and the intercompany flag from the configured partner mapping. CANCELED is kept (N/Y/C); gold filters it.
-- partition_by: period
