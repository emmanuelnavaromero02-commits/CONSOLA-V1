...
# field (vendor banking, customer SSN, finance master data) with a
# publicly known key. The cartridge now refuses to start instead.
_FIELD_ENCRYPTION_KEY = os.environ.get("FIELD_ENCRYPTION_KEY", "").strip()
if not _FIELD_ENCRYPTION_KEY:
    raise RuntimeError(
        "FIELD_ENCRYPTION_KEY is required for SAP Business One protection_service. "
...
try:
    ...
    raise RuntimeError(
        f"FIELD_ENCRYPTION_KEY invalid for SAP Business One: {exc}. "
