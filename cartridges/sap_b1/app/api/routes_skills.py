...
def _external_failure(exc: Exception, entity: str | None = None) -> JSONResponse:
    ...
    payload = {
        ...
        "message": "SAP Business One database request failed.",
    }
    if entity:
...
def test_connection(
    ...
    try:
        ...
        return B1Client(security_context=x_security_context).test_connection()
...
def list_tables() -> dict:
    ...
    return {"tables": B1Client().list_tables()}
...
def get_table_schema(table_id: str) -> dict:
    ...
    return B1Client().get_table_schema(table_id)
