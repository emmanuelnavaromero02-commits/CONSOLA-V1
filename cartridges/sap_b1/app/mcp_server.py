...
===========================
Exposes the cartridge tools over Streamable HTTP so that Claude (or any MCP
client) can inspect, extract and query SAP Business One data without
writing custom code.
...
def _b1_allowed_kb_prefixes() -> tuple[str, str, str]:
    bucket = settings.minio_bucket
    return (
        f"s3://{bucket}/raw/sap_b1/",
        f"s3://{bucket}/silver/sap_b1/",
        f"s3://{bucket}/gold/sap_b1/",
    )
...
def _watermarks_by_entity() -> dict[str, dict[str, str]]:
    ...
    try:
        ...
        return grouped
    for row in rows:
        name, _, alias = str(row.get("entity_name") or "").partition("@")
        if name and alias and row.get("last_watermark_value"):
            grouped.setdefault(name, {})[alias] = str(row["last_watermark_value"])
    return grouped

mcp = FastMCP(
    name="sap_b1",
    instructions=(
        "You have access to the SAP Business One cartridge: one company per "
        "schema, tables named as in Business One (OINV, INV1, OJDT, OINM...). "
        ...
        "Rows carry _company (the company alias) and _source_updated_at."
    ),
)
...
def list_entities() -> list[dict[str, Any]]:
    """
    List all SAP Business One entities with their extraction mode, watermark field,
    last recorded watermark value, and description.
    """
    entities = get_all_entities()
    marks = _watermarks_by_entity()
    result = []
    for e in entities:
        ...
        result.append(
            {
                ...
                "watermark_field": wf,
                "last_watermark": marks.get(name) or None,
...
def get_schema(entity: str) -> dict[str, Any]:
    """
    Return the configuration schema for a SAP Business One entity including field list,
    watermark config, and extraction mode.

    Args:
        entity: Entity name as listed by list_entities (e.g. "OINV", "INV1")
    """
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    """
    Preview the most recent rows for a SAP Business One entity from Bronze (MinIO Parquet).
    Returns column names and up to `limit` rows.

    Args:
        entity: Entity name (e.g. "OINV", "OITM")
    ...
    path = f"s3://{bucket}/raw/sap_b1/{entity}/{scope}load_date=*/batch_id=*/*.parquet"
...
async def extract(
    ...
    """
    [BATCH — async] Trigger extraction of a SAP Business One entity into Bronze (MinIO Parquet).
...
async def extract_all(mode: str = "incremental") -> dict[str, Any]:
    """
    [BATCH — async] Extrae TODAS las entidades de SAP Business One en paralelo (máx 4 simultáneas).
...
async def get_run_logs(job_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    rows = await pool.fetch(
        ...
        "FROM run_logs WHERE run_id=$1 AND cartridge='sap_b1' "
...
def list_kbs() -> list[dict[str, Any]]:
    """
    List all Knowledge Bits defined for the SAP Business One cartridge, including
...
def run_kb(kb_id: str) -> dict[str, Any]:
    ...
    Args:
        kb_id: Knowledge Bit ID as listed by list_kbs() (e.g. "kb_sales_by_company_month")
    """
    try:
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    """
    Run read-only DuckDB SQL against SAP Business One Bronze/Silver/Gold Parquet data.
    ...
    Args:
        sql:   DuckDB SQL query. Wrap table refs like:
               read_parquet('s3://{bucket}/raw/sap_b1/OINV/**/*.parquet')
...
def _make_sql_tool(name: str, description: str, sql: str) -> None:
    ...
    ok, err = validate_kb_sql(resolved_sql, _b1_allowed_kb_prefixes())
    if not ok:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "WHERE cartridge_id='sap_b1' AND enabled=TRUE"
                )
                rows = cur.fetchall()
        finally:
            conn.close()
