...
app = FastAPI(title="SAP Business One Cartridge", lifespan=lifespan)
...
async def _unhandled_exception_handler(request: Request, exc: Exception):
    request_id = _log_internal_error(exc, "unhandled sap_b1 exception", request)
    return JSONResponse(
...
async def mcp_invoke(body: dict, request: Request):
    ...
    try:
        ...
        request_id = _log_internal_error(exc, "sap_b1 mcp invoke failed", request)
        return JSONResponse(
            {"error": "Internal Error", "request_id": request_id},
            status_code=500,
        )
