...
def test_banxico_watermark_rls_migration_scopes_runtime_role() -> None:
    sql = (
        Path(__file__).resolve()
        .parents[3]
        .joinpath("infra/init/99zr_banxico_entity_watermarks_rls.sql")
        .read_text(encoding="utf-8")
    )
    assert "omega_cartridge_banxico" in sql
    assert "entity_watermarks_banxico_workspace_rls" in sql
