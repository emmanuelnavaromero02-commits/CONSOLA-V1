...
_SERVICE_HEADERS: tuple[str, ...] = ("banxico", "cartridge-banxico")
...
def _auth_options() -> list[tuple[str, str]]:
    ...
    key = os.environ.get("INTERNAL_API_KEY_BANXICO_TO_CONSOLE", "")
    if key:
        ...
        for key, internal_service in _auth_options():
            try:
                ...
                response = requests.get(
                    f"{console_url}/api/vault/connections/banxico/{candidate_conn_id}/reveal",
                    headers=headers,
                    timeout=5,
                )
...
def resolve_banxico_token(*, conn_id: str | None = None, security_context: str | None = None) -> str:
    env_token = os.environ.get("BANXICO_API_TOKEN", "").strip()
    if env_token and _local_fallbacks_allowed():
        return env_token

    selected_conn_id = _normalize_conn_id(conn_id)
    if not selected_conn_id:
        raise RuntimeError("Banxico Vault conn_id is required outside local development")
    payload = _fetch_connection(conn_id, security_context)
    for field in _FIELD_ALIASES:
        value = payload.get(field)
        if value is not None and str(value).strip():
            return str(value).strip()
    raise RuntimeError("Banxico token not found in Vault for the selected conn_id")
