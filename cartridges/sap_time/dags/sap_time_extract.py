"""
sap_time_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP Time Management API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_TIME_ANALYTICS={"conn_type":"http","host":"https://na5.sap_time.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_time_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_time' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_time_extract",
    description="Extrae una entidad de SAP Time Management → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_time", "bronze", "extract"],
    params={
...
def sap_time_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP Time ManagementClient(base_url, token)
