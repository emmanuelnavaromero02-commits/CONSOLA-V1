"""
sap_time_extract_all DAG  (Pattern B — portable, no custom container)
=====================================================================
Extrae TODAS las entidades del cartucho SAP Time Management en secuencia.
Útil para la carga inicial (mode=full) o para refreshes programados.

Credenciales en Airflow UI > Admin > Connections > +:
  sap_time_analytics: Conn Type=HTTP, Host=<base_url>, Password=<bearer_token>
  sap_time_services:  Conn Type=HTTP, Host=<base_url>, Password=<bearer_token>
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "WHERE cartridge_id='sap_time' AND entity_name=%s", (entity,)
            )
            row = cur.fetchone()
        conn.close()
...
def _load_entities(override_names: set | None = None) -> list[dict]:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            "FROM entity_config "
            "WHERE cartridge_id='sap_time' AND enabled=TRUE "
            "ORDER BY entity",
        )
        rows = cur.fetchall()
    conn.close()
...
#   Conn Id: sap_time_analytics  |  Conn Type: HTTP
#   Host: https://na5.sap_time.com/analyticsapi  |  Password: <bearer_token>

def _get_connection(conn_id: str) -> tuple[str, str]:
    """Return (base_url, token) from Airflow connection sap_time_<conn_id>."""
    from airflow.hooks.base import BaseHook
    airflow_conn_id = f"sap_time_{conn_id}"
    try:
        ...
        raise ValueError(
            ...
            f"  Host: https://na5.sap_time.com/analyticsapi  |  Password: <bearer_token>"
        )
...
def _upload_parquet(df, entity: str, run_id: str) -> str:
    ...
    key    = f"raw/sap_time/{entity}/load_date={today}/batch_id={run_id}/{entity}.parquet"
...
# ── SAP Time Management HTTP client ──────────────────────────────────────────────────────

class _SAP Time ManagementClient:
    RETRY     = 5
...
def _extract_one(client: _SAP Time ManagementClient, entity: str,
...
@dag(
    dag_id="sap_time_extract_all",
    description="Extrae TODAS las entidades de SAP Time Management → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_time", "bronze", "extract", "full"],
    params={
...
def sap_time_extract_all():
    ...
    def extract_all(params: dict = None) -> dict:
        ...
        if not entity_list:
            raise ValueError(
                "No hay entidades registradas para el cartucho 'sap_time'. "
                "Ejecuta el seed SQL: infra/init/02_sap_time_seed.sql"
            )

        # Validate all needed connections are configured in Vault
        needed_conns = {e["connection"] for e in entity_list}
        missing = []
        conn_cache: dict[str, _SAP Time ManagementClient] = {}

        for conn_id in needed_conns:
            try:
                base_url, token = _get_connection(conn_id)
                conn_cache[conn_id] = _SAP Time ManagementClient(base_url, token)
