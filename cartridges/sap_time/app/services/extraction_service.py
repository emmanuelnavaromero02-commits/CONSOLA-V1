...
# Guards against SAP Time Management clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP Time Management's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP Time Management entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP Time Management, then split into batches.
