...
PREFIX = "s3://lakehouse/raw/salesforce/"
PREFIXES = (
    PREFIX,
    "s3://lakehouse/silver/salesforce/",
    "s3://lakehouse/gold/salesforce/",
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'SELECT * FROM read_parquet("s3://lakehouse/raw/salesforce/Account/**/*.parquet")',
        "SELECT * FROM read_parquet('s3://lakehouse/silver/salesforce/salesforce_opportunity_latest/*.parquet')",
        "SELECT * FROM read_parquet('s3://lakehouse/gold/salesforce/salesforce_pipeline_forecast/*.parquet')",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("SELECT * FROM read_parquet('s3://lakehouse/raw/salesforce/../secret.parquet')", "traversal"),
        ("SELECT * FROM read_parquet('s3://other/raw/salesforce/x.parquet')", "path must start"),
        ("SELECT * FROM read_parquet('s3://lakehouse/raw/replicon/x.parquet')", "path must start"),
...
def test_query_kb_error_response_does_not_leak_resolved_sql_or_paths(monkeypatch):
    ...
    class LeakyConnection:
        def execute(self, sql: str):
            raise RuntimeError(f"boom while reading {sql} from s3://lakehouse/raw/salesforce/Opportunity/x.parquet")
    ...
    result = mcp_server.query_kb(
        "SELECT * FROM read_parquet('s3://{bucket}/raw/salesforce/Opportunity/*.parquet')"
    )
...
def test_existing_salesforce_kb_sql_passes_guard():
    repo_root = Path(__file__).resolve().parents[3]
    kb_path = repo_root / "cartridges" / "salesforce" / "app" / "config" / "knowledge_bits.yaml"
