...
def _validate_date(value: str, param: str) -> None:
    if not _ISO_DATE_RE.match(value):
        raise HTTPException(
            status_code=422, detail=f"{param} must be ISO-8601 date/datetime"
        )
...
def _external_failure(exc: Exception, entity: str | None = None) -> JSONResponse:
    ...
    payload = {
        ...
        "message": "Salesforce upstream request failed.",
    }
    if entity:
...
# triggering an extraction. SalesforceClient.test_connection() is degraded-aware.
...
def test_connection(
    ...
    try:
        ...
        return SalesforceClient(security_context=x_security_context).test_connection()
...
def list_tables() -> dict:
    ...
    return {"tables": SalesforceClient().list_tables()}
...
def get_table_schema(table_id: str) -> dict:
    ...
    return SalesforceClient().get_table_schema(table_id)
