...
def check_salesforce(security_context: str | None = None) -> dict[str, Any]:
    return SalesforceClient(security_context=security_context).configuration_status()
...
def preflight_for_extract(security_context: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    components = [check_salesforce(serialized), check_postgres(), check_minio()]
