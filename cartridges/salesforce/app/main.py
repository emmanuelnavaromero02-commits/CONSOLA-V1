...
app = FastAPI(title="Salesforce Cartridge", lifespan=lifespan)
...
async def _unhandled_exception_handler(request: Request, exc: Exception):
    request_id = _log_internal_error(exc, "unhandled salesforce exception", request)
    return JSONResponse(
...
app.add_middleware(
    ...
    paths=(r"/entities/[A-Za-z0-9_.-]+/extract", r"/extract-all"),
)
...
async def mcp_invoke(body: dict, request: Request):
    ...
    try:
        ...
        request_id = _log_internal_error(exc, "salesforce mcp invoke failed", request)
        return JSONResponse(
            {"error": "Internal Error", "request_id": request_id},
            status_code=500,
        )
