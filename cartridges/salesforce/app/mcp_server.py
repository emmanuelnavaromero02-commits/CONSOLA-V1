...
mcp = FastMCP(
    name="salesforce",
    instructions=(
        "You have access to the Salesforce workforce-management cartridge. "
...
def list_entities() -> list[dict[str, Any]]:
    """
    List all Salesforce entities with their extraction mode, watermark field,
...
def get_schema(entity: str) -> dict[str, Any]:
    """
    Return the configuration schema for a Salesforce entity including field list,
    watermark config, and extraction mode.

    Args:
        entity: Entity name as listed by list_entities (e.g. "User", "Opportunity")
    """
    config = get_entity_config(entity)
    if not config:
        return {"error": f"Entity '{entity}' not found"}
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    """
    Preview the most recent rows for a Salesforce entity from Bronze (MinIO Parquet).
    ...
    scope = scoped_prefix()
    path = (
        f"s3://{bucket}/raw/salesforce/{entity}/{scope}load_date=*/batch_id=*/*.parquet"
    )
...
async def extract(
    ...
    """
    [BATCH — async] Trigger extraction of a Salesforce entity into Bronze (MinIO Parquet).
...
async def extract_all(mode: str = "incremental") -> dict[str, Any]:
    """
    [BATCH — async] Extrae TODAS las entidades de Salesforce en paralelo (máx 4 simultáneas).
...
async def get_run_logs(job_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    rows = await pool.fetch(
        ...
        "FROM run_logs WHERE run_id=$1 AND cartridge='salesforce' "
...
def list_kbs() -> list[dict[str, Any]]:
    """
    List all Knowledge Bits defined for the Salesforce cartridge, including
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    """
    Run read-only DuckDB SQL against Salesforce Bronze/Silver/Gold Parquet data.
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "WHERE cartridge_id='salesforce' AND enabled=TRUE"
                )
                rows = cur.fetchall()
        finally:
            conn.close()
