...
def _external_failure(exc: Exception, entity: str | None = None) -> JSONResponse:
    ...
    message = "HubSpot upstream rejected the request."
    if upstream_status not in (401, 403):
        message = "HubSpot upstream request failed."
    payload = {
        "status": "failed",
...
def run_incremental(entity: str, body: dict[str, Any] | None = Body(None)) -> dict:
    ...
    try:
        ...
        return _run_entity_with_context({**config, "mode": mode}, body)
    except Exception as exc:
        return _external_failure(exc, entity)
...
def run_historical_load(
    ...
    """
    Date-range load for entities with a date_field / watermark_field.

    Args:
        entity:    object id (e.g. deals)
    ...
    if not config:
        raise HTTPException(status_code=404, detail=f"Entity not found: {entity}")
    if not (config.get("date_field") or config.get("watermark_field")):
        raise HTTPException(
            status_code=400,
            detail=f"Entity {entity} has no date_field/watermark_field. Use run_full_load instead.",
        )
    try:
...
def run_historical_load_all(
    ...
    """Date-range load for all entities that have a date_field / watermark_field."""
    results = []
    for config in get_all_entities():
        if not (config.get("date_field") or config.get("watermark_field")):
            continue
...
def list_tables() -> dict:
    """Return all available HubSpot entities with their descriptions."""
    from app.core.hubspot_client import HubSpotClient
