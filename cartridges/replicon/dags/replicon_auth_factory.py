...
def _method(value: Any, default: str = "bearer_token") -> str:
    raw = str(value or default).strip().lower().replace("-", "_")
    if raw in {"bearer", "bearer_token", "token"}:
        return "bearer_token"
    if raw in {"api_key", "apikey", "x_api_key"}:
        return "api_key"
    if raw in {"basic", "basic_auth"}:
        return "basic"
    if raw in {"none", "no_auth", "anonymous"}:
        return "none"
...
def build_auth_headers(
    ...
    *,
    ...
    method = _method(payload.get("auth_method"), default_method)
