...
# Guards against SAP Check-In Empleados clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP Check-In Empleados's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP Check-In Empleados entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP Check-In Empleados, then split into batches.
