"""
sap_checkin_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP Check-In Empleados API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_CHECKIN_ANALYTICS={"conn_type":"http","host":"https://na5.sap_checkin.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_checkin_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_checkin' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_checkin_extract",
    description="Extrae una entidad de SAP Check-In Empleados → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_checkin", "bronze", "extract"],
    params={
...
def sap_checkin_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP Check-In EmpleadosClient(base_url, token)
