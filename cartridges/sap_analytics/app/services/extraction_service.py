...
# Guards against SAP Analytics clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP Analytics's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP Analytics entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP Analytics, then split into batches.
