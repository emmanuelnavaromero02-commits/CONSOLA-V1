"""
SAP Analytics Batch Job Runner
=========================
...
async def cleanup_stale() -> None:
    ...
    try:
        ...
        await pool.execute(
            ...
            "finished_at=NOW() WHERE status='running' AND tool LIKE 'sap_analytics__%'"
        )
    except Exception:
        pass
...
async def _log(
    ...
    try:
        ...
        await pool.execute(
            ...
            "VALUES ($1, 'sap_analytics', $2, $3, $4, $5::jsonb)",
...
async def create_extract_job(
    ...
    - Si AIRFLOW_URL está configurado: delega al DAG sap_analytics_extract en Airflow.
    ...
    await _insert(job_id, "sap_analytics__extract", args)
...
async def create_extract_all_job(mode: str = "incremental") -> dict:
    ...
    """
    job_id = str(uuid.uuid4())[:8]
    await _insert(job_id, "sap_analytics__extract_all", {"mode": mode})
...
async def list_jobs(limit: int = 10) -> list[dict]:
    ...
    rows = await pool.fetch(
        "SELECT * FROM jobs WHERE tool LIKE 'sap_analytics__%' "
...
async def _trigger_silver_refresh(entity: str) -> None:
    ...
    """
    source = f"raw/sap_analytics/{entity}"
    try:
...
async def _trigger_airflow(
    ...
    """POST to Airflow REST API to trigger the sap_analytics_extract DAG."""
    ...
    conf = {
        ...
        "sap_analytics_base_url": settings.sap_analytics_base_url,
    }
    url = f"{settings.airflow_url}/api/v1/dags/sap_analytics_extract/dagRuns"
