"""
sap_analytics_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP Analytics API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_ANALYTICS_ANALYTICS={"conn_type":"http","host":"https://na5.sap_analytics.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_analytics_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_analytics' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_analytics_extract",
    description="Extrae una entidad de SAP Analytics → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_analytics", "bronze", "extract"],
    params={
...
def sap_analytics_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP AnalyticsClient(base_url, token)
