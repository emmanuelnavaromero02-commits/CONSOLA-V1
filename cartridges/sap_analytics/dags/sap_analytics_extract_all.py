"""
sap_analytics_extract_all DAG  (Pattern B — portable, no custom container)
=====================================================================
Extrae TODAS las entidades del cartucho SAP Analytics en secuencia.
Útil para la carga inicial (mode=full) o para refreshes programados.

Credenciales en Airflow UI > Admin > Connections > +:
  sap_analytics_analytics: Conn Type=HTTP, Host=<base_url>, Password=<bearer_token>
  sap_analytics_services:  Conn Type=HTTP, Host=<base_url>, Password=<bearer_token>
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "WHERE cartridge_id='sap_analytics' AND entity_name=%s", (entity,)
            )
            row = cur.fetchone()
        conn.close()
        ...
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO entity_watermarks
                   ...
                   VALUES ('sap_analytics', %s, %s, %s, %s, NOW())
...
def _load_entities(override_names: set | None = None) -> list[dict]:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            "FROM entity_config "
            "WHERE cartridge_id='sap_analytics' AND enabled=TRUE "
            "ORDER BY entity",
        )
        rows = cur.fetchall()
    conn.close()
...
#   Conn Id: sap_analytics_analytics  |  Conn Type: HTTP
#   Host: https://na5.sap_analytics.com/analyticsapi  |  Password: <bearer_token>

def _get_connection(conn_id: str) -> tuple[str, str]:
    """Return (base_url, token) from Airflow connection sap_analytics_<conn_id>."""
    from airflow.hooks.base import BaseHook
    airflow_conn_id = f"sap_analytics_{conn_id}"
    try:
        ...
        raise ValueError(
            ...
            f"  Host: https://na5.sap_analytics.com/analyticsapi  |  Password: <bearer_token>"
        )
...
def _upload_parquet(df, entity: str, run_id: str) -> str:
    ...
    key    = f"raw/sap_analytics/{entity}/load_date={today}/batch_id={run_id}/{entity}.parquet"
...
# ── SAP Analytics HTTP client ──────────────────────────────────────────────────────

class _SAP AnalyticsClient:
    RETRY     = 5
...
def _extract_one(client: _SAP AnalyticsClient, entity: str,
...
@dag(
    dag_id="sap_analytics_extract_all",
    description="Extrae TODAS las entidades de SAP Analytics → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_analytics", "bronze", "extract", "full"],
    params={
...
def sap_analytics_extract_all():
    ...
    def extract_all(params: dict = None) -> dict:
        ...
        if not entity_list:
            raise ValueError(
                "No hay entidades registradas para el cartucho 'sap_analytics'. "
                "Ejecuta el seed SQL: infra/init/02_sap_analytics_seed.sql"
            )

        # Validate all needed connections are configured in Vault
        needed_conns = {e["connection"] for e in entity_list}
        missing = []
        conn_cache: dict[str, _SAP AnalyticsClient] = {}

        for conn_id in needed_conns:
            try:
                base_url, token = _get_connection(conn_id)
                conn_cache[conn_id] = _SAP AnalyticsClient(base_url, token)
