...
def _hcm_allowed_kb_prefixes() -> tuple[str, str, str]:
    bucket = settings.minio_bucket
    return (
        f"s3://{bucket}/raw/sap_hcm/",
        f"s3://{bucket}/silver/sap_hcm/",
        f"s3://{bucket}/gold/sap_hcm/",
    )
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    ...
    bucket = settings.minio_bucket
    scope = scoped_prefix()
    path = f"s3://{bucket}/raw/sap_hcm/{entity}/{scope}load_date=*/batch_id=*/*.parquet"
...
async def get_run_logs(job_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    rows = await pool.fetch(
        ...
        "FROM run_logs WHERE run_id=$1 AND cartridge='sap_hcm' "
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    """
    Run read-only DuckDB SQL against SAP HCM Bronze/Silver/Gold Parquet data.
    ...
    ok, err = validate_kb_sql(resolved, _hcm_allowed_kb_prefixes())
    if not ok:
