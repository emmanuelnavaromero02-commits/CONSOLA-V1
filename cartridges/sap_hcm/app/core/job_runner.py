...
async def cleanup_stale() -> None:
    ...
    try:
        ...
        await pool.execute(
            ...
            "finished_at=NOW() WHERE status='running' AND tool LIKE 'sap_hcm__%'"
        )
    except Exception:
        pass
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute(
                ...
                "VALUES ($1, 'sap_hcm', $2, $3, $4, $5::jsonb, $6::uuid, $7::uuid, 'scoped')",
...
async def create_extract_job(
    ...
    - Si AIRFLOW_URL está configurado: delega al DAG sap_hcm_extract en Airflow.
...
async def list_jobs(limit: int = 10) -> list[dict]:
    ...
    async with pool.acquire() as conn:
        ...
        rows = await conn.fetch(
            "SELECT * FROM jobs WHERE tool LIKE 'sap_hcm__%' "
...
async def _trigger_airflow(
    ...
    """POST to Airflow REST API to trigger the sap_hcm_extract DAG."""
