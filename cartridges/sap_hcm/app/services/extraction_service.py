...
# Guards against SAP HCM Core clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP HCM Core's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP HCM Core entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP HCM Core, then split into batches.
