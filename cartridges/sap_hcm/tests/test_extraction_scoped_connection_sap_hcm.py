...
def test_extraction_resolves_vault_credentials_per_tenant(monkeypatch):
    ...
    credentials = {
        "tenant-a": {
            "base_url": "https://sap-hcm-a.example.test",
            "user": "user-tenant-a",
            "password": "password-tenant-a",
        },
        "tenant-b": {
            "base_url": "https://sap-hcm-b.example.test",
            "user": "user-tenant-b",
    ...
    monkeypatch.setattr(client_module.SapHcmClient, "fetch_entity", fake_fetch_entity)
    ...
    for tenant_id, workspace_id in (("tenant-a", "ws-a"), ("tenant-b", "ws-b")):
        ...
        result = extraction_service.run_entity(
            {"entity": "Employee", "mode": "full", "security_context": ctx}
        )
