"""
sap_hcm_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP HCM Core API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_HCM_ANALYTICS={"conn_type":"http","host":"https://na5.sap_hcm.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_hcm_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_hcm' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_hcm_extract",
    description="Extrae una entidad de SAP HCM Core → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_hcm", "bronze", "extract"],
    params={
...
def sap_hcm_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP HCM CoreClient(base_url, token)
