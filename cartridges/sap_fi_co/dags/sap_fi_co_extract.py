"""
sap_fi_co_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP FI/CO Finanzas API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_FICO_ANALYTICS={"conn_type":"http","host":"https://na5.sap_fi_co.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_fi_co_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_fi_co' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_fi_co_extract",
    description="Extrae una entidad de SAP FI/CO Finanzas → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_fi_co", "bronze", "extract"],
    params={
...
def sap_fi_co_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP FI/CO FinanzasClient(base_url, token)
