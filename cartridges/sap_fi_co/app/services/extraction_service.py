...
# Guards against SAP FI/CO Finanzas clock skew or late-arriving records.
...
def _apply_watermark_filter(
    ...
    SAP FI/CO Finanzas's download-type extract does not support server-side date filters,
...
def run_entity(
    ...
    """
    Extract one SAP FI/CO Finanzas entity (table) and write it to Bronze (MinIO Parquet).
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Extract full table from SAP FI/CO Finanzas, then split into batches.
