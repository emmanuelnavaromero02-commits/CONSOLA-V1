"""
SAP FI/CO Finanzas Batch Job Runner
=========================
...
async def cleanup_stale() -> None:
    ...
    try:
        ...
        await pool.execute(
            ...
            "finished_at=NOW() WHERE status='running' AND tool LIKE 'sap_fi_co__%'"
        )
    except Exception:
        pass
...
async def _log(
    ...
    try:
        ...
        await pool.execute(
            ...
            "VALUES ($1, 'sap_fi_co', $2, $3, $4, $5::jsonb)",
...
async def create_extract_job(
    ...
    - Si AIRFLOW_URL está configurado: delega al DAG sap_fi_co_extract en Airflow.
    ...
    await _insert(job_id, "sap_fi_co__extract", args)
...
async def create_extract_all_job(mode: str = "incremental") -> dict:
    ...
    """
    job_id = str(uuid.uuid4())[:8]
    await _insert(job_id, "sap_fi_co__extract_all", {"mode": mode})
...
async def list_jobs(limit: int = 10) -> list[dict]:
    ...
    rows = await pool.fetch(
        "SELECT * FROM jobs WHERE tool LIKE 'sap_fi_co__%' "
...
async def _trigger_airflow(
    ...
    """POST to Airflow REST API to trigger the sap_fi_co_extract DAG."""
