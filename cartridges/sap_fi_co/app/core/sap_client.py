...
class SAP FI/CO FinanzasClient:
    """
    Client for the SAP FI/CO Finanzas Analytics BI API.

    Authentication: static Bearer token from SAP_FICO_TOKEN env var.
    ...
    def __init__(self) -> None:
        self.base_url = settings.sap_fi_co_base_url.rstrip("/")
        self._token = settings.sap_fi_co_api_token

        if not settings.use_demo_data and not self._token:
            raise EnvironmentError("SAP_FICO_TOKEN is required (set in .env)")
    ...
    def _poll_extract(self, extract_id: str) -> dict[str, Any]:
        ...
        deadline = time.monotonic() + settings.sap_fi_co_poll_timeout
        while time.monotonic() < deadline:
            ...
            time.sleep(settings.sap_fi_co_poll_interval)

        raise TimeoutError(
            f"Extract {extract_id} did not complete within {settings.sap_fi_co_poll_timeout}s"
        )
    ...
    def extract_table(self, table_id: str) -> list[dict[str, Any]]:
        """
        Full extract of one SAP FI/CO Finanzas table. Returns list[dict].
