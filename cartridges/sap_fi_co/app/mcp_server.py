"""
SAP FI/CO Finanzas MCP Server
===================
Exposes 8 tools over Streamable HTTP so that Claude (or any MCP client)
can inspect, extract, and query SAP FI/CO Finanzas data without writing custom code.
...
mcp = FastMCP(
    name="sap_fi_co",
    instructions=(
        "You have access to the SAP FI/CO Finanzas workforce-management cartridge. "
...
def list_entities() -> list[dict[str, Any]]:
    """
    List all SAP FI/CO Finanzas entities with their extraction mode, watermark field,
...
def get_schema(entity: str) -> dict[str, Any]:
    """
    Return the configuration schema for a SAP FI/CO Finanzas entity including field list,
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    """
    Preview the most recent rows for a SAP FI/CO Finanzas entity from Bronze (MinIO Parquet).
    ...
    path = f"s3://{bucket}/raw/sap_fi_co/{entity}/load_date=*/batch_id=*/*.parquet"
...
async def extract(
    ...
    """
    [BATCH — async] Trigger extraction of a SAP FI/CO Finanzas entity into Bronze (MinIO Parquet).
...
async def extract_all(mode: str = "incremental") -> dict[str, Any]:
    """
    [BATCH — async] Extrae TODAS las entidades de SAP FI/CO Finanzas en paralelo (máx 4 simultáneas).
...
async def get_run_logs(job_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    rows = await pool.fetch(
        ...
        "FROM run_logs WHERE run_id=$1 AND cartridge='sap_fi_co' "
...
def list_kbs() -> list[dict[str, Any]]:
    """
    List all Knowledge Bits defined for the SAP FI/CO Finanzas cartridge, including
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    """
    Run arbitrary DuckDB SQL against SAP FI/CO Finanzas Bronze/Silver Parquet data.
    ...
    Args:
        sql:   DuckDB SQL query. Wrap table refs like:
               read_parquet('s3://{bucket}/raw/sap_fi_co/TimeEntry/**/*.parquet')
