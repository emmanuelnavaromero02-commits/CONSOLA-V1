...
class SAP PayrollClient:
    """
    Client for the SAP Payroll Analytics BI API.

    Authentication: static Bearer token from SAP_PAYROLL_TOKEN env var.
    ...
    def __init__(self) -> None:
        self.base_url = settings.sap_payroll_base_url.rstrip("/")
        self._token = settings.sap_payroll_api_token

        if not settings.use_demo_data and not self._token:
            raise EnvironmentError("SAP_PAYROLL_TOKEN is required (set in .env)")
    ...
    def _poll_extract(self, extract_id: str) -> dict[str, Any]:
        ...
        deadline = time.monotonic() + settings.sap_payroll_poll_timeout
        while time.monotonic() < deadline:
            ...
            time.sleep(settings.sap_payroll_poll_interval)

        raise TimeoutError(
            f"Extract {extract_id} did not complete within {settings.sap_payroll_poll_timeout}s"
        )
    ...
    def extract_table(self, table_id: str) -> list[dict[str, Any]]:
        """
        Full extract of one SAP Payroll table. Returns list[dict].
