"""
SAP Payroll MCP Server
===================
Exposes 8 tools over Streamable HTTP so that Claude (or any MCP client)
can inspect, extract, and query SAP Payroll data without writing custom code.
...
mcp = FastMCP(
    name="sap_payroll",
    instructions=(
        "You have access to the SAP Payroll workforce-management cartridge. "
...
def list_entities() -> list[dict[str, Any]]:
    """
    List all SAP Payroll entities with their extraction mode, watermark field,
...
def get_schema(entity: str) -> dict[str, Any]:
    """
    Return the configuration schema for a SAP Payroll entity including field list,
...
def preview(entity: str, limit: int = 20) -> dict[str, Any]:
    """
    Preview the most recent rows for a SAP Payroll entity from Bronze (MinIO Parquet).
    ...
    path = f"s3://{bucket}/raw/sap_payroll/{entity}/load_date=*/batch_id=*/*.parquet"
...
async def extract(
    ...
    """
    [BATCH — async] Trigger extraction of a SAP Payroll entity into Bronze (MinIO Parquet).
...
async def extract_all(mode: str = "incremental") -> dict[str, Any]:
    """
    [BATCH — async] Extrae TODAS las entidades de SAP Payroll en paralelo (máx 4 simultáneas).
...
async def get_run_logs(job_id: str, limit: int = 50) -> list[dict[str, Any]]:
    ...
    rows = await pool.fetch(
        ...
        "FROM run_logs WHERE run_id=$1 AND cartridge='sap_payroll' "
...
def list_kbs() -> list[dict[str, Any]]:
    """
    List all Knowledge Bits defined for the SAP Payroll cartridge, including
...
def query_kb(sql: str, limit: int = 100) -> dict[str, Any]:
    """
    Run arbitrary DuckDB SQL against SAP Payroll Bronze/Silver Parquet data.
    ...
    Args:
        sql:   DuckDB SQL query. Wrap table refs like:
               read_parquet('s3://{bucket}/raw/sap_payroll/TimeEntry/**/*.parquet')
