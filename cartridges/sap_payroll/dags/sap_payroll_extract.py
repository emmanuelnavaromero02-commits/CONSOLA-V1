"""
sap_payroll_extract DAG  (Pattern B — portable, no custom container)
=================================================================
Extrae UNA entidad de SAP Payroll API → Parquet → MinIO (Bronze).
...
#   AIRFLOW_CONN_SAP_PAYROLL_ANALYTICS={"conn_type":"http","host":"https://na5.sap_payroll.com/analyticsapi","password":"TOKEN"}
#
# Campos: Conn Id = sap_payroll_<conn_id> | Conn Type = HTTP
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                "FROM entity_config WHERE cartridge_id='sap_payroll' AND entity=%s",
                (entity,),
            )
            row = cur.fetchone()
        conn.close()
...
@dag(
    dag_id="sap_payroll_extract",
    description="Extrae una entidad de SAP Payroll → MinIO Bronze Parquet",
    default_args=default_args,
    schedule=None,
    catchup=False,
    tags=["sap_payroll", "bronze", "extract"],
    params={
...
def sap_payroll_extract():
    ...
    def extract(params: dict = None) -> dict:
        ...
        client = _SAP PayrollClient(base_url, token)
