...
from .errors import ChecksumMismatch, ObjectAlreadyExists, ObjectNotFound, StorageError, UnsupportedPrecondition, UnsafePrefixDelete
...
class GCSStorage:
    ...
    def _client_or_create(self):
        if self._client is None:
            try:
                ...
                raise StorageError("google-cloud-storage is required for GCS provider", provider="gcs") from exc
    ...
    def _blob(self, key: str):
        return self._bucket().blob(validate_key(key))

    def uri_for(self, key: str) -> str:
        return f"gs://{self.config.bucket}/{validate_key(key)}"

    def _metadata(self, metadata: Mapping[str, str] | None, checksum: str) -> dict[str, str]:
    ...
    def _generation(self, expected_version: str | None, *, overwrite: bool) -> int | None:
        if expected_version:
            try:
                return int(expected_version)
            except ValueError as exc:
                raise UnsupportedPrecondition("GCS expected_version must be a numeric generation", provider="gcs") from exc
    ...
    def stat(self, key: str) -> ObjectStat:
        ...
        try:
            ...
            if self._not_found(exc):
                raise ObjectNotFound("object not found", provider="gcs", bucket=self.config.bucket, key=key) from exc
            raise StorageError("object stat failed", provider="gcs", bucket=self.config.bucket, key=key) from exc
    ...
    def put_bytes(self, key: str, data: bytes, **kwargs) -> PutResult:
        ...
        if checksum != sha256_bytes(data):
            raise ChecksumMismatch("provided checksum does not match payload", provider="gcs", bucket=self.config.bucket, key=key)
        overwrite = bool(kwargs.get("overwrite", False))
        generation = self._generation(kwargs.get("expected_version"), overwrite=overwrite)
        if not overwrite and self.exists(key):
            raise ObjectAlreadyExists("object already exists", provider="gcs", bucket=self.config.bucket, key=key)
        blob = self._blob(key)
        blob.metadata = self._metadata(kwargs.get("metadata"), checksum)
        blob.upload_from_string(data, content_type="application/octet-stream", if_generation_match=generation)
        return self._result(key, len(data), checksum, kwargs.get("metadata"))

    def put_file(self, key: str, path: Path, **kwargs) -> PutResult:
        path = Path(path)
        return self.put_bytes(key, path.read_bytes(), **{**kwargs, "checksum_sha256": kwargs.get("checksum_sha256") or sha256_file(path)})

    def _result(self, key: str, size: int, checksum: str, metadata: Mapping[str, str] | None) -> PutResult:
        version = None
        try:
            version = self.stat(key).version
        except StorageError:
            pass
        return PutResult(key, self.uri_for(key), size, checksum, version, metadata or {})
    ...
    def get_bytes(self, key: str) -> bytes:
        ...
        try:
            ...
            if self._not_found(exc):
                raise ObjectNotFound("object not found", provider="gcs", bucket=self.config.bucket, key=key) from exc
            raise StorageError("object read failed", provider="gcs", bucket=self.config.bucket, key=key) from exc
    ...
    def iter_chunks(self, key: str, *, chunk_size: int = 1024 * 1024) -> Iterator[bytes]:
        with self.open_reader(key) as fh:
            while True:
    ...
    def copy(self, source_key: str, target_key: str, **kwargs) -> PutResult:
        ...
        if not overwrite and self.exists(target_key):
            raise ObjectAlreadyExists("object already exists", provider="gcs", bucket=self.config.bucket, key=target_key)
        source = self.stat(source_key)
        self._bucket().copy_blob(self._blob(source_key), self._bucket(), target_key, if_generation_match=generation)
        return self._result(target_key, source.size, source.checksum_sha256 or "", kwargs.get("metadata") or source.metadata)
    ...
    def publish(self, staging_key: str, final_key: str, **kwargs) -> PublishResult:
        ...
        if expected_sha and staging_sha != expected_sha:
            raise ChecksumMismatch("staging checksum mismatch", provider="gcs", bucket=self.config.bucket, key=staging_key)
        result = self.copy(staging_key, final_key, overwrite=bool(kwargs.get("overwrite", False)), expected_version=kwargs.get("expected_version"))
        ...
        if expected_sha and final_sha != expected_sha:
            raise ChecksumMismatch("published checksum mismatch", provider="gcs", bucket=self.config.bucket, key=final_key)
    ...
    def list_page(self, prefix: str, *, page_size: int = 1000, cursor: str | None = None, delimiter: str | None = None) -> ListPage:
        ...
        return ListPage(objects, iterator.next_page_token, bool(iterator.next_page_token), prefixes)

    def _stat_from_blob(self, blob) -> ObjectStat:
        key = str(blob.name)
        metadata = {str(k).lower(): str(v) for k, v in (blob.metadata or {}).items()}
        return ObjectStat(key, self.uri_for(key), int(blob.size or 0), blob.updated, blob.etag, str(blob.generation), metadata.get(_HASH_METADATA), metadata)
    ...
    def delete_prefix(self, prefix: str, *, require_trailing_slash: bool = True, max_objects: int | None = None) -> int:
        ...
        if max_objects is not None and len(keys) > max_objects:
            raise UnsafePrefixDelete("delete_prefix exceeded max_objects", provider="gcs", bucket=self.config.bucket, key=prefix)
        for key in keys:
            self.delete_object(key)
        return len(keys)

    def presigned_get_url(self, key: str, *, expires_in: int = 300) -> str:
        return self._blob(key).generate_signed_url(expiration=timedelta(seconds=expires_in), method="GET")
