...
if [[ "$lock_file" != /* || -e "$lock_file" || -L "$lock_file" || \
      -e "$authority_file" || -L "$authority_file" ]]; then
  echo "ERROR: lock and authority files must be new absolute, non-symlink paths." >&2
  exit 6
fi
...
if [[ "${OMEGA_GHCR_PREFLIGHT_TEST_MODE:-0}" == "1" && \
      -n "${OMEGA_GHCR_PREFLIGHT_TEST_AUTHORITY_FILE:-}" ]]; then
  cp -- "${OMEGA_GHCR_PREFLIGHT_TEST_AUTHORITY_FILE}" "$temp_authority"
elif ! python3 "$release_helper" "${authority_args[@]}" >/dev/null; then
...
mv -f -- "$temp_authority" "$authority_file"
temp_authority=""
python3 - "$lock_dir" <<'PY'
import os, sys
