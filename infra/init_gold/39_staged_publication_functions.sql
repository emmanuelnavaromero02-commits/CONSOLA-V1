...
BEGIN
  ...
  IF current_run.layer = 'gold' THEN
    ...
    SELECT jsonb_agg(
             ...
             ORDER BY a.attnum
           ) INTO authoritative_catalog
      FROM pg_attribute a
     WHERE a.attrelid=format('omega_publication_stage.%I',p_staging_table)::regclass
       AND a.attnum > 0 AND NOT a.attisdropped;
    IF authoritative_catalog IS NULL OR jsonb_array_length(authoritative_catalog)=0 THEN
    ...
    IF (SELECT jsonb_agg(item->>'name' ORDER BY ordinal)
       ...
       (SELECT jsonb_agg(item->>'name' ORDER BY ordinal)
          FROM jsonb_array_elements(authoritative_catalog)
  ...
  ELSE
    ...
    IF jsonb_array_length(authoritative_catalog)=0 THEN
      RAISE EXCEPTION 'Silver catalog is empty' USING ERRCODE='23514';
    END IF;
  END IF;
