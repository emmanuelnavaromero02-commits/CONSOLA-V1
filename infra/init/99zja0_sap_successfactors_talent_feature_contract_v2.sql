-- WB-TALENTO operational feature contract v2 (continued by 99zja_*).
UPDATE datasets
   SET sources = '[
...
metrics AS (
    SELECT
        ...
        COALESCE((SELECT BOOL_AND(benchmark_approval_valid = TRUE AND benchmark_provenance_status = 'approved_durable') FROM readiness WHERE source_mode = 'benchmark_internal'), FALSE) AS benchmark_approval_valid,
...
SELECT
    ...
    CASE WHEN readiness_benchmark_count = 0 THEN 'not_applicable' WHEN benchmark_approval_valid THEN 'approved_durable' ELSE 'unreviewed' END AS benchmark_provenance_status,
    ...
    CASE WHEN readiness_benchmark_count > 0 AND readiness_cpa_real_count = 0 THEN NULL ELSE confidence END AS confidence,
    CASE
