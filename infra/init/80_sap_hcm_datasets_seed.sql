...
-- Mirrors infra/init/10_replicon_gold_seed.sql: the per-dataset SQL bodies live
-- in cartridges/sap_hcm/datasets/*.sql (the source of truth for review) and are
-- inlined here as dollar-quoted literals so refinement can materialize them.
--
-- 13 silver + 8 gold = 21 datasets. Idempotent: ON CONFLICT (name) DO NOTHING.
-- Datasets are registered via this infra migration (not the cartridge seed.sql)
-- because the cartridge seed guard intentionally disallows INSERT INTO datasets
-- and INSERT ... SELECT -- same approach Replicon uses for its catalog.
...
$seed$, $seed$Última extracción de EmployeeMaster (PA0001) con campos tipados, filtrada por el load_date más reciente.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM latest
ORDER BY pernr
$seed$, $seed$Última extracción de PersonalData (PA0002) con campos tipados. Nombre y fecha de nacimiento llegan ya protegidos desde bronze (masked / encrypted).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM latest
ORDER BY pernr, valid_from
$seed$, $seed$Última extracción de EmployeeActions (PA0000): altas, bajas y traslados, con campos tipados.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM latest
ORDER BY pernr, valid_from
$seed$, $seed$Última extracción de ContractData (PA0016): elementos de contrato por empleado.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Última extracción de OrgUnit (HRP1000 Otype=O): unidades organizacionales con id y nombre.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Última extracción de Position (HRP1000 Otype=S): posiciones con id y nombre.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Última extracción de JobCode (HRP1000 Otype=C): trabajos / clasificaciones con id y nombre.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Última extracción de CostCenter (subconjunto de PA0001 con Kostl) con campos tipados.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM latest
ORDER BY pernr, valid_from
$seed$, $seed$Última extracción de LeaveAbsence (PA2001): ausencias por empleado, tipo y días.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM latest
ORDER BY pernr, valid_from
$seed$, $seed$Última extracción de WorkSchedule (PA0007): horario de trabajo planificado por empleado.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
WHERE e.rn = 1
ORDER BY e.pernr
$seed$, $seed$Vista 360 del empleado: asignación org (PA0001) + datos personales (PA0002) + contrato (PA0016), enriquecida con nombre de unidad org y posición. Una fila por empleado (registro vigente).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
WHERE rn = 1
ORDER BY org_id
$seed$, $seed$Estructura organizacional. Lista las unidades org con columnas de jerarquía. El padre queda NULL hasta habilitar HRP1001 (relaciones OM), que no se extrae hoy.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Posiciones con su titular actual (si existe). Marca posiciones vacantes (sin empleado asignado vía Plans).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Empleados activos por unidad organizacional (snapshot del mes en curso).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Empleados activos por centro de costo (snapshot del mes en curso).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Distribución de empleados activos por tipo de personal (employee_group / subgroup de PA0001). Proxy de "tipo de posición" hasta que se extraiga la clasificación de puesto.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Días de ausencia por empleado y tipo en los últimos 12 meses. Una fila por (pernr, tipo de ausencia).$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Tendencia mensual de ausencias por tipo: total de días hábiles y empleados distintos afectados.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
FROM emp
ORDER BY pernr
$seed$, $seed$Árbol de supervisión por empleado. El vínculo manager no está disponible en el bronze actual (PA0001 extraído no incluye Sbrtr y no se extrae HRP1001); se entrega cada empleado activo con manager NULL hasta habilitar esa fuente.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Detección automática de irregularidades operativas sobre empleados activos (preparación Fase 3). UNION de varios casos con tipo, severidad y detalle.$seed$, $seed${}$seed$::jsonb, NULL, NOW()),
...
$seed$, $seed$Costo de nómina estimado por mes / unidad org / centro de costo. PENDIENTE: requiere extracción de PA0008 (BasicPay), no habilitada en Bloque A. Hoy entrega la dimensión con headcount y costo en NULL.$seed$, $seed${}$seed$::jsonb, NULL, NOW())
