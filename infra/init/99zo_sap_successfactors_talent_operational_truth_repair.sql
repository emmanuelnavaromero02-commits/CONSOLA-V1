-- Fail-closed repair for previously materialized Talent benchmark projections.
-- A recomputation from the v3 packaged datasets is required to regain readiness.
...
BEGIN
    FOREACH schema_name IN ARRAY ARRAY['public', 'pggold'] LOOP
        relation_name := 'gold_sap_successfactors_talent_benchmark_internal';
        qualified_name := format('%I.%I', schema_name, relation_name);
        IF to_regclass(qualified_name) IS NOT NULL THEN
            EXECUTE format(
            ...
            EXECUTE format(
                ...
                'approval_status = ''unreviewed'', approval_source = ''system_default'', '
        ...
        relation_name := 'gold_sap_successfactors_talent_readiness';
        qualified_name := format('%I.%I', schema_name, relation_name);
        IF to_regclass(qualified_name) IS NOT NULL THEN
            EXECUTE format(
        ...
        relation_name := 'gold_sap_successfactors_talent_9box';
        qualified_name := format('%I.%I', schema_name, relation_name);
        IF to_regclass(qualified_name) IS NOT NULL THEN
            EXECUTE format(
                'ALTER TABLE %s ADD COLUMN IF NOT EXISTS benchmark_approval_valid BOOLEAN NOT NULL DEFAULT FALSE',
...
VALUES ('99zo_sap_successfactors_talent_operational_truth_repair.sql', NOW())
