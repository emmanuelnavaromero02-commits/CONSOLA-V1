from __future__ import annotations

from contextlib import asynccontextmanager
from unittest.mock import AsyncMock

import pytest
from fastapi import HTTPException

from app.services import copilot_context_authority as authority
from app.services import copilot_context_service as service


TENANT_ID = "11111111-1111-1111-1111-111111111111"
WORKSPACE_ID = "22222222-2222-2222-2222-222222222222"
VIEWER = {
    "id": 7,
    "role": "viewer",
    "active_tenant_id": TENANT_ID,
    "active_workspace_id": WORKSPACE_ID,
}
OPERATOR = {**VIEWER, "id": 8, "role": "tenant_admin"}
WRITER = {**VIEWER, "id": 9, "role": "workspace_admin"}


def _row(fingerprint: str, required_permission: str) -> dict[str, object]:
    return {
        "id": "33333333-3333-3333-3333-333333333333",
        "fingerprint": fingerprint,
        "title": "Revisar señal",
        "required_permission": required_permission,
        "status": "active",
    }


def _install_db(monkeypatch, connection) -> tuple[AsyncMock, AsyncMock]:
    pool = AsyncMock(return_value=object())
    audit = AsyncMock()
    monkeypatch.setattr(authority.auth, "pool", pool)
    monkeypatch.setattr(authority, "tables_ready", AsyncMock(return_value=True))
    monkeypatch.setattr(authority.audit_service, "record_event", audit)

    @asynccontextmanager
    async def scoped_db(*_args, **_kwargs):
        yield connection

    monkeypatch.setattr(authority, "scoped_db", scoped_db)
    return pool, audit


@pytest.mark.asyncio
async def test_list_filters_each_recommendation_by_declared_permission(
    monkeypatch,
) -> None:
    class Connection:
        async def fetch(self, sql: str, *args):
            assert "required_permission = ANY($3::text[])" in sql
            assert "LIMIT $4" in sql
            assert args[3] == 20
            return [
                _row("live:monitor", "monitor.read"),
                _row("live:operations", "operations.read"),
                _row("live:unknown", "unknown.permission"),
            ]

    _install_db(monkeypatch, Connection())

    viewer = await service.list_recommendations(VIEWER)
    operator = await service.list_recommendations(OPERATOR)

    assert [row["fingerprint"] for row in viewer["recommendations"]] == ["live:monitor"]
    assert [row["fingerprint"] for row in operator["recommendations"]] == [
        "live:monitor",
        "live:operations",
    ]


@pytest.mark.asyncio
async def test_dismiss_applies_the_same_permission_inside_atomic_update(
    monkeypatch,
) -> None:
    class Connection:
        mutated = False
        calls: list[tuple[str, tuple[object, ...]]] = []

        async def fetchrow(self, sql: str, *args):
            self.calls.append((sql, args))
            if "SELECT required_permission" in sql:
                return {"required_permission": "operations.read"}
            self.mutated = True
            assert "required_permission = ANY($4::text[])" in sql
            assert "operations.read" in args[3]
            return {
                "id": "33333333-3333-3333-3333-333333333333",
                "fingerprint": "live:operations",
                "status": "dismissed",
            }

    connection = Connection()
    pool, audit = _install_db(monkeypatch, connection)

    with pytest.raises(HTTPException) as denied:
        await service.dismiss_recommendation(VIEWER, "live:operations")
    assert denied.value.status_code == 403
    pool.assert_not_awaited()
    assert connection.calls == []
    audit.assert_not_awaited()

    with pytest.raises(HTTPException) as denied:
        await service.dismiss_recommendation(WRITER, "live:operations")
    assert denied.value.status_code == 403
    assert connection.mutated is False
    assert len(connection.calls) == 1
    assert "FOR UPDATE" in connection.calls[0][0]

    result = await service.dismiss_recommendation(
        OPERATOR,
        "live:operations",
        ip="127.0.0.1",
        user_agent="pytest",
    )

    assert result["status"] == "dismissed"
    assert connection.mutated is True
    assert len(connection.calls) == 3
    audit.assert_awaited_once()
    audit_kwargs = audit.await_args.kwargs
    assert audit_kwargs["connection"] is connection
    assert audit_kwargs["critical"] is True
    assert audit_kwargs["user_id"] == OPERATOR["id"]
    assert audit_kwargs["ip"] == "127.0.0.1"
    assert audit_kwargs["metadata"] == {
        "tenant_id": TENANT_ID,
        "workspace_id": WORKSPACE_ID,
        "fingerprint": "live:operations",
        "required_permission": "operations.read",
    }


@pytest.mark.asyncio
async def test_dismiss_fails_closed_when_transactional_audit_fails(
    monkeypatch,
) -> None:
    class Connection:
        async def fetchrow(self, sql: str, *_args):
            if "SELECT required_permission" in sql:
                return {"required_permission": "monitor.read"}
            return {
                "id": "33333333-3333-3333-3333-333333333333",
                "fingerprint": "live:monitor",
                "status": "dismissed",
            }

    connection = Connection()
    _, audit = _install_db(monkeypatch, connection)
    audit.side_effect = RuntimeError("critical audit failed")

    with pytest.raises(RuntimeError, match="critical audit failed"):
        await service.dismiss_recommendation(WRITER, "live:monitor")

    assert audit.await_args.kwargs["connection"] is connection
    assert audit.await_args.kwargs["critical"] is True


@pytest.mark.asyncio
@pytest.mark.parametrize("persist", [False, True])
async def test_refresh_rejects_operations_only_before_pool_or_collectors(
    monkeypatch,
    persist: bool,
) -> None:
    pool = AsyncMock()
    collector = AsyncMock()
    monkeypatch.setattr(service.auth, "pool", pool)
    monkeypatch.setattr(service.control_room_service, "ops_summary", collector)
    monkeypatch.setattr(
        authority.permissions,
        "has_permission",
        lambda _user, permission: permission == "operations.read",
    )

    with pytest.raises(HTTPException) as denied:
        await service.collect_workspace_context(OPERATOR, persist=persist)

    assert denied.value.status_code == 403
    pool.assert_not_awaited()
    collector.assert_not_awaited()


def test_talent_metadata_recommendation_requires_operations_read() -> None:
    recommendations = service.build_recommendations_from_snapshot(
        {
            "sources": [
                {
                    "name": "control_room.sap_successfactors_talent_metadata_readiness",
                    "data": {
                        "status": "partial",
                        "summary": {
                            "blocked_entities": 1,
                            "live_required_total": 2,
                            "live_required_ready": 1,
                        },
                    },
                }
            ]
        }
    )

    metadata = next(
        item for item in recommendations if "metadata" in item["title"].lower()
    )
    assert metadata["required_permission"] == "operations.read"
