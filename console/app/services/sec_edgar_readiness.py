...
DATASET = "sec_market_context"


async def sec_edgar_readiness(user: dict | None) -> dict[str, Any]:
    try:
        rows = await query_gold_dataset_rows(DATASET, user, limit=100)
    except HTTPException as exc:
        if exc.status_code == 404:
            return _empty("insufficient_data", "sec_market_context_not_materialized")
        raise
