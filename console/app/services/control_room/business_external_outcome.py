...
def adapter_error_outcome_is_ambiguous(
    ...
    *,
    ...
    if isinstance(error, (AdapterCircuitOpenError, AdapterConfigurationError)):
        return False
...
async def ambiguous_unknown_error_response(
    conn: Any,
    *,
    remote_attempt_started: bool,
    workspace_id: str,
    ...
    detail = {
        **dict(result),
        ...
        "reconciliation_required": True,
        "message": (
            "External write-back may have executed; the result requires "
            "reconciliation with the same idempotency key."
        ),
    }
    return {"_http_error_status": 409, "_http_error_detail": detail}
