...
# Rango de privilegio de los roles globales de IAM. Un actor NUNCA puede
# asignar/crear un rol de rango ESTRICTAMENTE mayor que el suyo (evita que un
# admin fabrique un owner/super_admin — escalada vertical). Los roles no
# listados (workspace/user) tienen rango 0 y no participan de este tope.
_ROLE_RANK = {
    "owner": 4,
    "super_admin": 3,
    "admin": 2,
...
# Roles de workspace que confieren gestion IAM (leer/administrar usuarios) en
# ESE workspace. viewer/analyst/user NO estan aqui: por eso un admin en A que
# es mero viewer en B no debe poder ver ni tocar usuarios de B.
...
def iam_admin_workspace_ids(user: Mapping[str, Any] | None) -> set[str]:
    """Workspaces donde el usuario tiene un rol que le permite administrar
    usuarios (no solo pertenecer). Es el SCOPE correcto de un admin de
    workspace: cierra el leak horizontal sin tocar la resolucion del candidato.
    """
    return {
        str(workspace.get("workspace_id"))
        for workspace in ((user or {}).get("workspaces") or [])
...
def assignable_role(
    ...
    *,
    ...
    if is_global_iam_admin(actor_user, role_admin=role_admin):
        # Tope de rango: ni un admin global puede asignar un rol de rango
        # superior al suyo. owner->owner y super_admin->admin siguen OK;
        # admin->owner / admin->super_admin quedan bloqueados (escalada).
