import { state } from './legacy-state.js';
    ...
    export async function extractNow(cartridge, entity, mode, dagId) {
      dagId = dagId || `${cartridge}_extract`;
        ...
        scFast.innerHTML = `
	      ...
	      try {
	        const [dsRes, srcRes] = await Promise.all([
	          fetchWithTimeout('/datasets'),
	          fetchWithTimeout('/api/sources'),
	        ]);
    ...
    export async function selectDS(name) {
      ...
      panel.innerHTML = _dsEditorHtml({
        ...
        lastRefresh: undefined,
	      ...
	      try {
	        const r = await fetchWithTimeout(`/api/datasets/${encodeURIComponent(name)}/detail`);
	        if (!r.ok) throw new Error(`HTTP ${r.status}`);
	        const d = await r.json();
...
FROM read_parquet('${upstream}', hive_partitioning=true, union_by_name=true)`;
    ...
    export async function loadDSDataPreview(name) {
      ...
      if (!area) return;
	      try {
	        const r = await fetchWithTimeout(`/datasets/${encodeURIComponent(name)}/data?limit=50`);
	        if (!r.ok) throw new Error(`HTTP ${r.status}`);
	        const d = await r.json();
